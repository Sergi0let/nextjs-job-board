  ...
  return (
    <aside className="flex w-[200px] flex-none flex-row items-center gap-2 md:flex-col md:items-stretch">
      {job.approved ? (
        ...
        <ApproveSubmissionBtn jobId={job.id} />
      )}
      <DeleteJobAdmin jobId={job.id} />
    </aside>
  );
}
...
function ApproveSubmissionBtn({ jobId }: AdminBtnProps) {
...
function DeleteJobAdmin({ jobId }: AdminBtnProps) {
